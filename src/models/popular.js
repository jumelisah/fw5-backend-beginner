...
exports.popularList = (data, cb)=>{
  db.query(`SELECT v.id AS id, v.name AS name, v.image AS image, v.location AS location, COUNT(h.vehicle_id) AS totalRent FROM vehicles v LEFT JOIN histories h ON v.id=h.vehicle_id WHERE name LIKE '%${data.vehicle_name}%' GROUP BY v.id ORDER BY totalRent DESC LIMIT ${data.limit} OFFSET ${data.offset}`, (err, res)=>{
    if(err) throw err;
    cb(res);
  });
};
...
exports.totalData = (data, cb)=>{
  db.query(`SELECT v.id AS id, v.name AS name, v.image AS image, COUNT(h.vehicle_id) AS totalRent FROM vehicles v LEFT JOIN histories h ON v.id=h.vehicle_id WHERE name LIKE '%${data.vehicle_name}%' GROUP BY v.id ORDER BY totalRent`, (err, res)=>{
    if(err) throw err;
    cb(res);
  });
};
