...
exports.getVehicles = (data)=>new Promise((resolve, reject)=>{
  db.query(`SELECT * FROM vehicles WHERE name LIKE '%${data.name}%' AND location LIKE '%${data.location}%' AND cost>=${data.cost_min} AND cost<=${data.cost_max} AND is_deleted=0 LIMIT ${data.limit} OFFSET ${data.offset}`, (err, res)=>{
    if(err) reject(err);
    resolve(res);
  });
});
...
exports.getVehicle = (id)=>new Promise((resolve, reject)=>{
  db.query(`SELECT * FROM vehicles WHERE id=${id} AND is_deleted=0`, (err, res)=>{
    if(err) reject(err);
    resolve(res);
  });
});
...
exports.getVehicleName = (data)=>new Promise((resolve, reject)=>{
  db.query(`SELECT * FROM vehicles WHERE name='${data.name}' && year='${data.year}' && cost=${data.cost} && location='${data.location}' && is_deleted=0`, (err, res)=>{
    if(err) reject(err);
    resolve(res);
  });
});
...
exports.deleteVehicle = (id)=>new Promise((resolve, reject)=>{
  db.query('UPDATE vehicles SET is_deleted=1 WHERE id=?', [id], (err, res)=>{
    if(err) reject(err);
    resolve(res);
  });
});